...
const ART = '/root/.claude/uploads/439b44da-b9ba-5210-9045-911a288ff5d9/5e90a303-image.jpg'
...
await page.pdf({
  path: path.join(OUT, 'v2-overlay.pdf'),
  width: mm(TRIM.w + BLEED * 2), height: mm(TRIM.h + BLEED * 2),
  printBackground: true, preferCSSPageSize: true,
})
await browser.close()
writeFileSync(path.join(OUT, 'v2-overlay.html'), html)
...
open('public/standee/v2-base.pdf','wb').write(base_bytes)

base = PdfReader('public/standee/v2-base.pdf')
ov   = PdfReader('public/standee/v2-overlay.pdf')
...
out = 'public/standee/marhaba-stand-v2.pdf'
