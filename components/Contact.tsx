...
export default function Contact() {
  ...
  // Create a form at https://formspree.io (deliver to info@wethink.ae), then set
  // NEXT_PUBLIC_FORMSPREE_ID in the deployment env (or paste the id string here).
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!FORMSPREE_ID) {
      // No form backend configured yet — open the visitor's email app pre-filled
      // instead of pretending the message was sent.
      const subject = encodeURIComponent(`Website enquiry — ${form.service || 'General'}`)
      const body = encodeURIComponent(
        `Name: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone}\nService: ${form.service}\n\n${form.message}`
      )
      window.location.href = `mailto:info@wethink.ae?subject=${subject}&body=${body}`
      return
    }
    ...
    try {
      ...
      setError('Something went wrong sending your message. Please email us directly at info@wethink.ae or reach us on WhatsApp.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        ...
        <div ref={ref} className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-6xl font-black mt-3 overflow-hidden">
            {[{ text: 'Start Your', gradient: false }, { text: 'Journey', gradient: true }].map((w, i) => (
              <motion.span
                key={w.text}
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-3">
                {[
                  ...
                  { name: 'LinkedIn', href: 'https://www.linkedin.com/in/rasha-aljalam-74a6b4188/', icon: <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" fill="currentColor" /> },
                ].map((s) => (
                  <a
                    key={s.name}
                    href={s.href}
                  ...
                  >
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="relative">
                        <label htmlFor="contact-name" className="block text-xs text-text-muted mb-1.5 font-medium uppercase tracking-wider">
                          Full Name *
                        </label>
                        <motion.input
                          id="contact-name"
                      ...
                      <div>
                        <label htmlFor="contact-email" className="block text-xs text-text-muted mb-1.5 font-medium uppercase tracking-wider">
                          Email Address *
                        </label>
                        <motion.input
                          id="contact-email"
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label htmlFor="contact-phone" className="block text-xs text-text-muted mb-1.5 font-medium uppercase tracking-wider">
                          Phone Number
                        </label>
                        <motion.input
                          id="contact-phone"
                      ...
                      <div>
                        <label htmlFor="contact-service" className="block text-xs text-text-muted mb-1.5 font-medium uppercase tracking-wider">
                          Service of Interest
                        </label>
                        <select
                          id="contact-service"
                    ...
                    {/* Message */}
                    <div>
                      <label htmlFor="contact-message" className="block text-xs text-text-muted mb-1.5 font-medium uppercase tracking-wider">
                        Your Message *
                      </label>
                      <motion.textarea
                        id="contact-message"
                    ...
                    {/* Error */}
                    {error && (
                      <p role="alert" className="text-sm rounded-lg px-4 py-3" style={{ color: '#FCA5A5', background: 'rgba(248,113,113,0.08)', border: '1px solid rgba(248,113,113,0.25)' }}>
                        {error}
                      </p>
                    )}
